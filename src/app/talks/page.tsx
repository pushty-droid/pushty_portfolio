    ...
    <>
        ...
        <section className="container-page pt-16">
          ...
          {upcoming.map((talk) => (
            <div key={talk.title} className="card border-l-2 border-l-accent p-6 sm:p-8">
              ...
              <h3 className="type-h3 mt-3 max-w-[36ch]">{talk.title}</h3>
      ...
      {past.length > 0 && (
        <section className="container-page pt-20">
          <SectionHeading eyebrow="Presented">Posters &amp; past talks</SectionHeading>
          <div className="space-y-10">
            {past.map((talk) => (
              <GutterEntry key={talk.title} label={talk.dates}>
                <h3 className="type-h3 max-w-[42ch]">{talk.title}</h3>
      ...
      <section className="container-page pt-20">
        <SectionHeading eyebrow="Recognition">Awards &amp; selections</SectionHeading>
        <ul className="divide-y divide-rule border-y border-rule">
          {awards.map((award) => (
            <li key={award.title} className="grid gap-x-8 gap-y-1 py-5 md:grid-cols-[9.5rem_1fr]">
      ...
      <section className="container-page pt-20">
        <SectionHeading eyebrow="Training">Schools &amp; workshops</SectionHeading>
        <div className="space-y-10">
          {schools.map((school) => (
            <GutterEntry key={school.programme} label={school.year}>
              <h3 className="type-h3 max-w-[42ch]">{school.programme}</h3>
        ...
        <ul className="grid gap-6 md:grid-cols-2">
          {outreach.map((item) => (
            <li key={item.organisation} className="card p-6">
              ...
              <h3 className="type-h3 mt-2">{item.organisation}</h3>
